...
import { sanitizePlain, validateEmail, validatePhone, validateInputLength, checkOrigin } from '../../../../lib/security';
...
export async function GET(request) {
  ...
  try {
    ...
    const res = await fetch(
      `${SUPABASE_URL}/rest/v1/profiles?select=*&order=created_at.desc&limit=${pageSize}&offset=${from}`,
      {
        headers: {
          apikey: SUPABASE_KEY,
    ...
    const total = parseInt(res.headers.get('content-range')?.split('/')[1] || profiles.length);
    return NextResponse.json({ data: profiles, total, page, pageSize, totalPages: Math.ceil(total / pageSize) });
  } catch {
