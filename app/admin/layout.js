  ...
  return (
    <ToastProvider>
      <div className="min-h-screen bg-[var(--color-bg)] flex" dir="rtl">
        ...
        {/* Sidebar */}
        <div className={`fixed lg:static z-50 transition-transform duration-300 lg:translate-x-0 ${sidebarOpen ? 'translate-x-0' : 'translate-x-full lg:translate-x-0'}`}>
          <Sidebar collapsed={collapsed} onToggle={() => setCollapsed(!collapsed)} user={user} />
        </div>

        {/* Main content */}
        <div className="flex-1 flex flex-col min-h-screen" style={{ marginRight: collapsed ? 'var(--sidebar-collapsed)' : 'var(--sidebar-width)' }}>
          {/* Topbar */}
