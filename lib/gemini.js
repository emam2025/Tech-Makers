...
async function callGemini(prompt) {
  ...
  let lastError = null;
  console.log('callGemini: Starting with', keys.length, 'keys');

  for (let i = 0; i < keys.length; i++) {
    const key = keys[i];
    const url = `${GEMINI_BASE}/${MODEL}:generateContent?key=${key}`;
    console.log(`callGemini: Trying key ${i + 1}, starts with: ${key?.substring(0, 15)}`);
    ...
    try {
      ...
      console.log(`callGemini: Key ${i + 1} status: ${res.status}`);
      ...
      if (!res.ok) {
        ...
        console.log(`callGemini: Key ${i + 1} error:`, errBody.substring(0, 200));
      ...
      if (!text) {
        console.log(`callGemini: Key ${i + 1} empty response`);
