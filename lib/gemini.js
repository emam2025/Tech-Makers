...
async function callGemini(prompt) {
  let lastError = null;
  console.log('callGemini: Starting with', API_KEYS.length, 'keys');

  for (let i = 0; i < API_KEYS.length; i++) {
    const key = API_KEYS[i];
    const url = `${GEMINI_BASE}/${MODEL}:generateContent?key=${key}`;
    console.log(`callGemini: Trying key ${i + 1}, key starts with: ${key?.substring(0, 10)}...`);
    ...
    try {
      ...
      console.log(`callGemini: Key ${i + 1} response status: ${res.status}`);

      if (res.status === 429) {
        lastError = new Error('Rate limit exceeded');
        console.log(`callGemini: Key ${i + 1} rate limited`);
        continue;
      }

      if (res.status === 401 || res.status === 403) {
        lastError = new Error(`Invalid API key (${res.status})`);
        console.log(`callGemini: Key ${i + 1} invalid`);
        continue;
      }

      if (!res.ok) {
        const errBody = await res.text();
        console.log(`callGemini: Key ${i + 1} error body:`, errBody.substring(0, 200));
      ...
      if (!text) {
        console.log(`callGemini: Key ${i + 1} empty response:`, JSON.stringify(data).substring(0, 200));
        lastError = new Error('Empty response from Gemini');
        continue;
      }

      console.log(`callGemini: Key ${i + 1} success`);
      return text;
    } catch (err) {
      console.log(`callGemini: Key ${i + 1} exception:`, err.message);
      lastError = err;
      continue;
    }
  }
...
Make questions progressively harder (A1 at start, C2 at end). Use natural, everyday English. Return ONLY the JSON, no other text.`;
  ...
  const text = await enqueue(() => callGemini(prompt));
