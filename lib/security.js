export async function rateLimit(ip, limit = 10, windowMs = 60000, endpoint = '/') {
  try {
    const supabaseUrl = process.env.SUPABASE_URL;
    const serviceKey = process.env.SUPABASE_SERVICE_KEY;

    if (!supabaseUrl || !serviceKey) {
      console.warn('Rate limit: Supabase credentials not configured');
      return true;
    }

    const now = new Date().toISOString();
    const windowStart = new Date(Date.now() - windowMs).toISOString();

    const baseUrl = `${supabaseUrl}/rest/v1/rate_limits`;
    const headers = {
      'Content-Type': 'application/json',
      apikey: serviceKey,
      Authorization: `Bearer ${serviceKey}`,
      Prefer: 'return=representation',
    };

    const q = `${baseUrl}?ip_address=eq.${encodeURIComponent(ip)}&endpoint=eq.${encodeURIComponent(endpoint)}&window_start=gte.${encodeURIComponent(windowStart)}&order=window_start.desc&limit=1`;
    const res = await fetch(q, { headers });

    if (res.ok) {
      const records = await res.json();
      if (records && records.length > 0) {
        const record = records[0];
        if (record.request_count >= limit) return false;

        await fetch(`${baseUrl}?id=eq.${record.id}`, {
          method: 'PATCH',
          headers: { ...headers, Prefer: 'return=minimal' },
          body: JSON.stringify({ request_count: record.request_count + 1 }),
        });
        return true;
      }
    }

    const insertRes = await fetch(baseUrl, {
      method: 'POST',
      headers,
      body: JSON.stringify({ ip_address: ip, endpoint, request_count: 1, window_start: now }),
    });

    return insertRes.ok;
  } catch (err) {
    console.error('Rate limit error:', err);
    return true;
  }
}

export function checkOrigin(request) {
  const origin = request.headers.get('origin');
  const host = request.headers.get('host');
  if (origin) {
    try {
      const originHost = new URL(origin).host;
      if (originHost === host) return true;
      return false;
    } catch { return false; }
  }
  const referer = request.headers.get('referer');
  if (referer) {
    try {
      const refererHost = new URL(referer).host;
      if (refererHost === host) return true;
      return false;
    } catch { return false; }
  }
  return false;
}

export function sanitize(str) {
  if (typeof str !== 'string') return '';
  return str
    .replace(/&/g, '&amp;')
    .replace(/</g, '&lt;')
    .replace(/>/g, '&gt;')
    .replace(/"/g, '&quot;')
    .replace(/'/g, '&#x27;');
}

export function sanitizePlain(str) {
  if (typeof str !== 'string') return '';
  return str.replace(/[<>"'&]/g, '');
}

export function validateEmail(email) {
  return /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email);
}

export function validatePhone(phone) {
  const cleaned = phone.replace(/[\s\-]/g, '');
  if (/^\+\d{7,15}$/.test(cleaned)) return true;
  if (/^0\d{9,14}$/.test(cleaned)) return true;
  return false;
}

export function validateName(name) {
  const trimmed = name.trim();
  if (trimmed.length < 5) return false;
  if (trimmed.length > 100) return false;
  return /^[\p{L}\s\-'()]+$/u.test(trimmed);
}

export function validateInputLength(data, limits = {}) {
  const defaults = { name: 100, email: 254, phone: 20, bio: 2000, portfolio: 500, certificate: 1000, specialty: 100, experience: 100, department: 100, country: 100 };
  const allLimits = { ...defaults, ...limits };
  for (const [key, maxLen] of Object.entries(allLimits)) {
    if (data[key] && typeof data[key] === 'string' && data[key].length > maxLen) {
      return `${key} يتجاوز الحد الأقصى (${maxLen} حرف)`;
    }
  }
  return null;
}

export function getClientIp(request) {
  const forwarded = request.headers.get('x-forwarded-for');
  if (forwarded) return forwarded.split(',')[0].trim();
  return request.headers.get('x-real-ip') || 'unknown';
}
